...
@pytest.fixture
def gumnet_model():
    model = GumNet(in_channels=1)
    model.eval()
    return model

@pytest.fixture
def gumnet_model_rgb():
    model = GumNet(in_channels=3)
    model.eval()
    return model
...
def test_model_initialization_default(gumnet_model):
    assert isinstance(gumnet_model, GumNet)
...
def test_affine_matrix_properties(gumnet_model):
    ...
    matrices_diff = torch.abs(affine_matrix[0] - affine_matrix[1]).sum()
    
    assert matrices_diff > 1e-6, "Affine matrices are suspiciously identical across batch"
...
def test_model_serialization():
    ...
    try:
        ...
        new_model = GumNet(in_channels=1)
