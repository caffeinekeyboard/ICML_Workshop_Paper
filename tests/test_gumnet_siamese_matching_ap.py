...
@pytest.fixture
def siamese_matcher():
    model = GumNetSiameseMatchingAP(in_channels=361, out_channels=1024)
    model.eval()
    return model
...
def test_model_initialization(siamese_matcher):
    assert isinstance(siamese_matcher, GumNetSiameseMatchingAP)
...
def test_device_agnosticism():
    model = GumNetSiameseMatchingAP(in_channels=361, out_channels=32) # Smaller channels for VRAM safety in tests
