...
@pytest.fixture
def siamese_matcher():
    model = GumNetSiameseMatching(in_channels=196, out_channels=1024)
    model.eval()
    return model
...
def test_model_initialization(siamese_matcher):
    assert isinstance(siamese_matcher, GumNetSiameseMatching)
...
def test_backward_pass_and_differentiability():
    model = GumNetSiameseMatching(in_channels=196, out_channels=1024)
    model.train()
    v_a = torch.randn(2, 512, 14, 14, requires_grad=True)
    v_b = torch.randn(2, 512, 14, 14, requires_grad=True)
...
def test_device_agnosticism():
    model = GumNetSiameseMatching(in_channels=196, out_channels=32) # Smaller channels for VRAM safety in tests
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    try:
        model = model.to(device)
        v_a = torch.randn(2, 512, 14, 14).to(device)
        v_b = torch.randn(2, 512, 14, 14).to(device)
...
def test_siamese_variable_batch_sizes(siamese_matcher):
    batch_1_a = torch.randn(1, 512, 14, 14)
    batch_1_b = torch.randn(1, 512, 14, 14)
    batch_4_a = torch.randn(4, 512, 14, 14)
    batch_4_b = torch.randn(4, 512, 14, 14)
