...
from model.gumnet_mp import GumNetMP
...
@pytest.fixture
def gumnet_model():
    model = GumNetMP(in_channels=1)
    model.eval()
    return model

@pytest.fixture
def gumnet_model_rgb():
    model = GumNetMP(in_channels=3)
    model.eval()
    return model
...
def test_model_initialization_default(gumnet_model):
    assert isinstance(gumnet_model, GumNetMP)
...
def test_model_serialization():
    ...
    try:
        ...
        new_model = GumNetMP(in_channels=1)
