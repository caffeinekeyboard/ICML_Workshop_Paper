...
from model.alternate.gumnet_siamese_matching_mp import GumNetSiameseMatchingMP
from model.alternate.gumnet_spatial_alignment_mp import GumNetSpatialAlignmentMP

class GumNetMP(nn.Module):
    """
    Complete GumNetMP architecture for fingerprint matching with spatial alignment.
    ...
    def __init__(self, in_channels=1):
        super(GumNetMP, self).__init__()

        self.feature_extractor = GumNetFeatureExtractionMP(in_channels=in_channels)
        self.siamese_matcher = GumNetSiameseMatchingMP()
        self.spatial_aligner = GumNetSpatialAlignmentMP()
